...
#   This interactive application allows researchers to calculate expected precision,
#   statistical power, and required sample sizes for time-to-event cohort studies 
#   that require adjustment for right-censoring and confounding bias.
#
# Core Architecture:
#   - User Interface (UI): MathJax-enabled fluid page with dedicated tabs for
#     Precision, Power, Sample Size calculation, and Methodological Validation.
#   - Server Logic: Reactive computational engine that dynamically recalculates
#     target estimands (Risk Difference, Risk Ratio) using closed-form solvers.
#   - Dependencies: Relies on `R/statistical_functions.R` for core mathematical
#     derivations (Greenwood's variance, Kish's design effect).
#
# Validation Integration:
#   The application loads pre-computed Monte Carlo simulation data (`results.csv`)
#   generated by `simulation_study.R` to populate the "Methodological Validation" 
#   tab, demonstrating the empirical validity of the underlying equations via 
#   interactive plots (ggiraph) and tables (DT).
#
# Usage:
#   Run locally via `shiny::runApp()` or deploy to a Shiny server. Ensure the 
#   Release Protocol is followed before updating production deployments.
# ==============================================================================
      ...
      conditionalPanel(
        ...
        conditionalPanel(
          ...
          sliderInput("RRrange", "Risk Ratio Range:",
                      min = 0.2, max = 5, value = c(1.0, 2.0), step = 0.1)
        ),
        conditionalPanel(
          condition = "input.calc_mode == 'sample_size'",
          sliderInput("RRrange_ss", "Risk Ratio Range:",
                      min = 0.2, max = 5, value = c(1.1, 2.0), step = 0.1)
        )
      ),
...
server <- function(input, output, session) {
  ...
  output$table_results <- renderDT({
    sim_data() %>%
      ...
      rename(`Group 1 Risk` = P_1, `Group 2 Risk` = P_2) %>%
      datatable(
        extensions = 'Buttons',
        options = list(
          dom = 'Bfrtip',
